        ...
        <TooltipPortal>
            <TooltipPositioner sideOffset={sideOffset} align={align} side={side}>
                <BaseTooltip.Popup
                    ...
                    className={cn(
                        "bg-popover text-popover-foreground outline-border z-50 w-fit origin-(--transform-origin) rounded-md px-3 py-1.5 text-xs text-balance shadow-sm outline -outline-offset-1 transition-[transform,scale,opacity] data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0",
                        className
                    )}
                    {...props}
                >
                    {children}
                    <TooltipArrow className="data-[side=bottom]:-top-2 data-[side=left]:right-[-13px] data-[side=left]:rotate-90 data-[side=right]:left-[-13px] data-[side=right]:-rotate-90 data-[side=top]:-bottom-2 data-[side=top]:rotate-180">
